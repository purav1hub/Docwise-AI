...
export const AnalysisDashboard: React.FC<AnalysisDashboardProps> = ({ data, onReset }) => {
  ...
  const analysisData = isComparison ? (data as ComparisonResult).docs[0] : (data as AnalysisResult);
    ...
    <div className="w-full max-w-5xl mx-auto pb-12 print:p-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 print:hidden">
        <div>
          ...
          <p className="text-slate-500">{isComparison ? `${(data as ComparisonResult).docs.length} Documents Compared` : analysisData.fileName}</p>
        </div>
      ...
      <div className="space-y-8">
        ...
        {(activeTab === 'one-page' || activeTab === 'comparison') && (
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm print:border-0 print:shadow-none">
                ...
                <p className="text-slate-700 leading-relaxed text-lg mb-6">{analysisData.onePageSummary}</p>
                <div className="grid grid-cols-2 gap-4">
                   <div className="p-4 bg-red-50 rounded-xl border border-red-100">
                      <span className="text-xs font-bold text-red-600 uppercase">Risk Level</span>
                      <div className="text-xl font-bold text-red-700">{analysisData.riskLevel}</div>
                   </div>
                   <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                      <span className="text-xs font-bold text-indigo-600 uppercase">Verdict</span>
                      <div className="text-xl font-bold text-indigo-700">{analysisData.verdict}</div>
                   </div>
                </div>
              </div>
              ...
              {isComparison && activeTab === 'comparison' && (
                <div className="bg-white p-8 rounded-2xl border border-brand-200 bg-brand-50/20 shadow-sm">
                  ...
                  <div className="p-4 bg-white rounded-xl border border-brand-100 mb-4">
                    ...
                    <div className="text-2xl font-black text-slate-800">{(data as ComparisonResult).winner}</div>
                  </div>
                  <p className="text-slate-600">{(data as ComparisonResult).winnerReason}</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                  {analysisData.importantDates.map((d, i) => (
                    <div key={i} className={`p-3 rounded-lg border ${d.deadline ? 'bg-orange-50 border-orange-200' : 'bg-slate-50 border-slate-100'}`}>
                      ...
                      <div className={`text-sm font-medium ${d.deadline ? 'text-orange-700' : 'text-slate-700'}`}>{d.event}</div>
                    </div>
                  ))}
              ...
              <div className="bg-slate-900 p-6 rounded-2xl text-white shadow-xl">
                 ...
                 <div className="flex items-end gap-3 mb-4">
                    <div className="text-4xl font-black">{analysisData.scamRiskScore}%</div>
                    <div className="text-xs font-bold opacity-60 mb-1">Risk Score</div>
                 </div>
                 <p className="text-xs opacity-80 leading-relaxed">{analysisData.scamAnalysis}</p>
              </div>
            </div>
          </div>
        )}
                ...
                <tbody className="divide-y divide-slate-100">
                  {(data as ComparisonResult).comparisonTable.map((row, i) => (
                    <tr key={i} className="hover:bg-slate-50/50">
                      <td className="p-4 text-sm font-bold text-slate-600">{row.feature}</td>
                      {row.values.map((val, vi) => (
                        <td key={vi} className="p-4 text-sm text-slate-600">{val}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
                 ...
                 <div className="space-y-2">
                    {analysisData.financialBreakdown.map((f, i) => (
                      <div key={i} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100">
                        <div>
                          <span className="text-xs font-bold text-slate-400 uppercase mr-2">{f.type}</span>
                ...
                <div key={idx} className={`p-6 rounded-2xl shadow-sm border ${flag.oneSided ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200'}`}>
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-3">
                       ...
                       <h4 className="font-bold text-slate-800 text-lg">{flag.title}</h4>
                    </div>
                  ...
                  <p className="text-slate-600 leading-relaxed mb-3">{flag.description}</p>
                  {flag.oneSided && (
    ...
    <div className="border border-slate-200 rounded-xl bg-white overflow-hidden shadow-sm">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors text-left">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-slate-700 text-sm">{clause.originalTitle}</span>
        </div>
        {isOpen ? <ChevronUp size={18} className="text-slate-400" /> : <ChevronDown size={18} className="text-slate-400" />}
      </button>
      {isOpen && (
        <div className="p-5 border-t border-slate-100 bg-slate-50/30">
          <p className="text-slate-600 text-sm leading-relaxed">{clause.simplifiedExplanation}</p>
        </div>
      )}
    </div>
  );
